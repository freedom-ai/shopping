...
function ec_group_addToCart(group,goodsId,parentId){
  ...
  Ajax.call('http://www.xianxiu.com/flow.php?step=add_to_cart_combo', 'goods=' + $.toJSON(goods), ec_group_addToCartResponse, 'POST', 'JSON'); //兼容jQuery by mike
}
//处理添加商品到购物车的反馈信息
...
function ec_group_delInCart(group,goodsId,parentId){
  ...
  goods.parent   = (typeof(parentId) == "undefined") ? 0 : parseInt(parentId);
  goods.group = group + '_' + group_item;//组名
  Ajax.call('http://www.xianxiu.com/flow.php?step=del_in_cart_combo', 'goods=' + $.toJSON(goods), ec_group_delInCartResponse, 'POST', 'JSON'); //兼容jQuery by mike
}
//处理删除购物车中的商品的反馈信息
...
function ec_group_openSpeDiv(message, group, goods_id, parent) 
{
  ...
  newMask.style.opacity = "http://www.xianxiu.com/0.40";
...
function addMultiToCart(group,goodsId){
	...
	Ajax.call('http://www.xianxiu.com/flow.php?step=add_to_cart_group', 'goods=' + $.toJSON(goods), addMultiToCartResponse, 'POST', 'JSON'); //兼容jQuery by mike
}
//回调
function addMultiToCartResponse(result){
	if(result.error > 0){
		...
		window.location.href = 'flow.php.htm'/*tpa=http://www.xianxiu.com/flow.php*/;
	}
}
//取消选项
...
//  }
  ...
  if (cmt.email.length > 0)
  {
   ...
   Ajax.call('http://www.xianxiu.com/comment.php', 'cmt=' + $.toJSON(cmt), commentResponse, 'POST', 'JSON');
   return false;
}
